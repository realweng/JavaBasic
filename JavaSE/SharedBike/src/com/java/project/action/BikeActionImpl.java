package com.java.project.action;

import com.java.project.action.Impl.BikeAction;
import com.java.demo.bean.*;
import com.java.project.constant.ConstantUtil;
import com.java.project.demo.bean.*;
import com.java.project.util.BikeUtil;
import com.java.project.util.DateUtil;
...
public class BikeActionImpl implements BikeAction {
            ...
            for (int i = 0;i < num;i++){ //添加几辆就循环扩容几次
                ...
                if(bikes instanceof OfoBike[]){
                    ...
                    ofoBikes[ofoBikes.length-1] = new OfoBike((ofoBikes.length+100),(bikeCompany.getBikeCompanyName()+
                            (ofoBikes.length+100)),1,null,null);
                    ...
                    moBikes[moBikes.length-1] = new MoBike((moBikes.length+300),(bikeCompany.getBikeCompanyName()+
                            (moBikes.length+100)),1,null,null);
                    ...
                    helloBikes[helloBikes.length-1] = new HelloBike((helloBikes.length+200),(bikeCompany.getBikeCompanyName()+
                            (helloBikes.length+100)),1,null,null);
    ...
    public void showBikes(BikeCompany[] bikeCompanies) {
        if (bikeCompanies != null) {
            for (int i = 0; i < bikeCompanies.length; i++) {
                ...
                System.out.println("-------------------------------------------");
                for (Bike bike : bikes) {
                    System.out.println(bike.getBid()+"\t"+"\t"+bike.getBname()+"\t"+ BikeUtil.getNameByStatus(bike.getStatus()) +"\t"+"\t"+bike.getBorrowTime()+"\t"+"\t"+bike.getReturnTime());
                    System.out.println("-------------------------------------------");
                }
            }
        }
        ...
        if(bikeCompany != null){//判断传入参数是否合法
            ...
            if(index == -1){
                ...
                if(status.equals(ConstantUtil.STATUS_ZERO)){//如果该车已经被借走了
                    ...
                    bikes[index].setBorrowTime(DateUtil.dateForMat());
        ...
        if(bikeCompany != null){
            ...
            if(index == -1){
                ...
                if(status.equals(ConstantUtil.STATUS_ONE)){
                    ...
                    String now = DateUtil.dateForMat();//当前还车时间
                    Date d1=sd.parse(bikes[index].getBorrowTime());//借车时间
                    Date d2=sd.parse(now);//当前还车时间
                    long charge =(d2.getTime()-d1.getTime())/(1000);
                    System.out.println("您借出了该车"+charge+"秒");
                    System.out.println("一共需要付款"+(charge*ConstantUtil.RENT)+"元！");
                    System.out.println("正在进行还车操作...");
                    bikes[index].setReturnTime(now);//设置还车时间为当前时间
    ...
    public void rankBike(BikeCompany[] bikeCompanies){
        BikeCompany[] companies = bikeCompanies; //临时存储原来数组里面的值
        if(bikeCompanies != null){
            for (int i = 0; i <companies.length - 1; i++) { //冒泡排序
                for (int j = 0; j < companies.length - i -1; j++) {
                    if(companies[j].getCount() < companies[j+1].getCount()){ //把借出次数大的排前面
            ...
            for (int i = 0; i < companies.length; i++) {
                System.out.println(companies[i].getBikeCompanyName()+"总借出次数："+companies[i].getCount()+"次");
            }
        }
    }
}